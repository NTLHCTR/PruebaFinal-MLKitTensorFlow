package com.example.pruebafinalmlkittensorflow;
...
import com.example.pruebafinalmlkittensorflow.ml.Modelfacultades;
        ...
        implements OnSuccessListener<Text>, OnFailureListener, ImageAnalysis.Analyzer, ImageReader.OnImageAvailableListener
{
    ...
    public void PersonalizedModel(View v) {
        try {
            Modelfacultades model = Modelfacultades.newInstance(getApplicationContext());
            TensorImage image = TensorImage.fromBitmap(mSelectedImage);

            Modelfacultades.Outputs outputs = model.process(image);
            ...
            for (int i = 0; i < probability.size(); i++) {
                if (probability.get(i).getScore()>= 0.4){
                    res = probability.get(i).getLabel() +  " " +  probability.get(i).getScore()*100 + " % \n";
                }
            }
        ...
        try {
            ...
            for (int i = 0; i < probability.size(); i++){
                if (probability.get(i).getScore()>= 0.5) {
                    res+=("Etiqueta: "+probability.get(i).getLabel() +  "\nConfianza: " +  probability.get(i).getScore()*100 + " %\n\n\n\n");
                } else if (probability.get(i).getScore()< 0.5) {
                    res+=("Etiqueta: No se ha podido reconocer la imagen\n\n\n\n\n");
                } /*if {
                    //txtResults.setText("Resultados: \nEtiqueta: No se ha podido reconocer la imagen\nConfianza: "+  probability.get(i).getScore()*100 + " %");
                    }*/
            }
            txtResults.setText("Resultados: \n"+ res);
            model.close();
